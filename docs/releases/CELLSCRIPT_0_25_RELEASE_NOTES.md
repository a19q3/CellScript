...
- Generic structs, enums, and pure functions now monomorphize deterministically
  ...
  cannot hide Cell-backed values.
...
emits a canonical `cellscript-package-interface-v1` record and
...
Metadata schema 60 adds `cellscript-typed-semantics-v1` and its hash. The record
contains canonical types, layouts, locals, operations, CFG, calls, effects,
ownership, borrow regions, and generic instantiations. Verified lowering record
v2 binds these facts to entry ABI and final machine blocks.
...
The project Inspector surfaces shortened public-interface and typed-semantics
hashes, while the Metadata tab retains the complete canonical records for
review or copying.

## Generated Surface And Failure Discipline

A compiler-owned executable-surface registry generates Markdown and JSON
support matrices. Strict production compilation rejects a known incomplete
shape before ASM or ELF generation with `E2105`; fail-closed runtime helpers
remain defense in depth rather than a public feature boundary.
