...
| LC-11 | Canonical module and public interface identity | implemented | `public`, `public(package)`, and `private`; canonical module/item identities; `cellscript-package-interface-v1`; interface hashes; dependency-facing templates; final gate verification remains. |
...
| LC-14 | Independently checked typed semantic record | implemented | `cellscript-typed-semantics-v1` records canonical types, locals, CFG operations, calls/effects, ownership, borrows, layouts, and instantiations; the parser-free checker validates it with V2419. Final gate verification remains. |
